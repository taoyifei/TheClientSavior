"""The Client Savior｜客户拯救者 Streamlit 入口。"""
...
def main() -> None:
    ...
    tab_case, tab_dashboard, tab_policies = st.tabs(
        ["AI 投诉处理", "管理看板", "政策库"]
    )
    with tab_case:
        _render_case_tab(policies, demo_cases)
    with tab_dashboard:
        _render_dashboard_tab()
    with tab_policies:
        _render_policy_tab(policies)
...
def _render_generation_status_bar(result: AgentResult) -> None:
    ...
    <div class="summary-grid">
        <div class="summary-card">
            ...
            <div class="metric-value">{html.escape(mode_text)}</div>
        </div>
        <div class="summary-card">
            <div class="metric-label">模型</div>
            <div class="metric-value">{html.escape(str(result.model))}</div>
        </div>
        <div class="summary-card">
            <div class="metric-label">耗时</div>
            <div class="metric-value">{result.elapsed_seconds:.2f} 秒</div>
        </div>
        <div class="summary-card">
            <div class="metric-label">缓存</div>
            <div class="metric-value">{html.escape(cache_text)}</div>
        </div>
    </div>
    """
...
def _render_script_cards(script: dict[str, str]) -> None:
    ...
    rows = st.columns(2)
    for index, (key, title) in enumerate(SCRIPT_SECTIONS):
        with rows[index % 2]:
            st.markdown(
                f"""
                <div class="script-card">
                    <b>{html.escape(title)}</b>
                    <span>{html.escape(str(script.get(key, "")))}</span>
                </div>
                """,
                unsafe_allow_html=True,
            )
...
def _policy_visible(
    ...
    if selected_category != "全部" and policy.get("category") != selected_category:
        return False
    if not query:
        return True
